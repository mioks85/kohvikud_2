...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    $search = $_GET['search'];
    $paring = "SELECT * FROM kohvikud WHERE nimi LIKE '%$search%' OR asukoht LIKE '%$search%' ORDER BY nimi ASC LIMIT $start, $limit";
} else {
    $sort = isset($_GET['sort']) ? $_GET['sort'] : 'nimi';
    $order = isset($_GET['order']) ? $_GET['order'] : 'ASC';
    $paring = "SELECT * FROM kohvikud ORDER BY $sort $order LIMIT $start, $limit";
}

$valjund = mysqli_query($yhendus, $paring);

// Lehekülgede navigatsioon
$paring_count = "SELECT COUNT(id) AS total FROM kohvikud";
